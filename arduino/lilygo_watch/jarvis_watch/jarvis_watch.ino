...
String gpsText = "GPS PELO CELULAR";
String cameraText = "CAMERA DO CELULAR";
...
void saveSettings(){
  ...
  prefs.putBool("wrist",wristWakeEnabled);
  prefs.putUShort("timeout",screenTimeoutSec);
  prefs.putUChar("power",(uint8_t)powerMode);
  prefs.putUChar("skin",(uint8_t)watchSkin);
  prefs.putBool("alarmOn",alarmEnabled);
  prefs.putUChar("alarmH",alarmHour);
  prefs.putUChar("alarmM",alarmMinute);
}
...
void loadSettings(){
  ...
  watchSkin=(WatchSkin)prefs.getUChar("skin",SKIN_CLASSIC);
  alarmEnabled=prefs.getBool("alarmOn",false);
  alarmHour=prefs.getUChar("alarmH",7);
  alarmMinute=prefs.getUChar("alarmM",0);
  brightnessLevel=constrain(brightnessLevel,40,255);
  if(powerMode>POWER_ULTRA)powerMode=POWER_ECO;
  if(watchSkin>=SKIN_COUNT)watchSkin=SKIN_CLASSIC;
  if(alarmHour>23)alarmHour=7;
  if(alarmMinute>59)alarmMinute=0;
}

void vibrateShort(){ if(vibrationEnabled&&watch&&watch->motor)watch->motor->onec(); }
void vibrateAlarm(){ if(!watch||!watch->motor)return; for(int i=0;i<6;i++){ watch->motor->onec(); delay(250); } }
...
void drawSkinClassic(){
  ...
  for(int i=0;i<60;i++){float a=i*6.0f*0.0174532925f;int r1=(i%5==0)?82:87;int x1=120+cos(a)*r1,y1=113+sin(a)*r1,x2=120+cos(a)*92,y2=113+sin(a)*92;tft->drawLine(x1,y1,x2,y2,(i%5==0)?C_WHITE:C_STEEL);}
  tft->setTextColor(C_WHITE,C_BLACK);tft->drawCentreString("JARVIS",120,33,2);tft->drawCentreString("CLASSIC",120,52,1);
  handFromAngle(120,113,48,(n.hour%12)*30+n.minute*.5f,C_WHITE,3);handFromAngle(120,113,72,n.minute*6,C_WHITE,2);handFromAngle(120,113,78,n.second*6,C_RED,1);tft->fillCircle(120,113,5,C_WHITE);
  ...
  tft->setTextColor(C_WHITE,C_BLACK);tft->drawCentreString("< skins | apps ^",120,222,1);
}
...
void drawSkinAnaDigi(){
  ...
  tft->fillRoundRect(23,122,92,72,6,C_BLACK);tft->fillRoundRect(125,122,92,72,6,C_LCD);tft->setTextColor(C_WHITE,C_BLACK);tft->drawCentreString("ANA-DIGI",69,132,1);tft->drawCentreString("ALM / TEMP",69,151,1);
  ...
  tft->setTextColor(C_WHITE,C_DARK);tft->drawCentreString("< skins | apps ^",120,216,1);
}
...
void drawHeader(const char*title){
  ...
  tft->setTextColor(C_TEXT,C_ORANGE);tft->drawString((twoDigits(n.hour)+":"+twoDigits(n.minute)).c_str(),49,7,4);tft->drawRightString(title,228,9,2);tft->drawString((twoDigits(n.day)+"/"+twoDigits(n.month)).c_str(),49,35,1);drawBatteryIcon(177,34,batt);tft->drawRightString(batt>=0?(String(batt)+"%").c_str():"--",228,34,1);
}
...
void drawApps(){
  drawHeader("APPS");
  lcarsButton(5,62,52,48,C_ORANGE,"VOZ");lcarsButton(62,62,52,48,C_SALMON,"ALM");lcarsButton(119,62,52,48,C_BLUE,"CAM");lcarsButton(176,62,59,48,C_GREEN,"GPS");
  lcarsButton(5,117,52,48,C_LAV,"CASA");lcarsButton(62,117,52,48,C_GOLD,"PASSOS");lcarsButton(119,117,52,48,C_BLUE,"STATUS");lcarsButton(176,117,59,48,C_SALMON,"CONFIG");
  tft->setTextColor(C_TEXT,C_BG);tft->drawCentreString("deslize para baixo: relogio",120,184,1);drawFooter();
}
...
void drawAlarm(){
  drawHeader("ALARME");tft->setTextColor(C_TEXT,C_BG);tft->drawCentreString((twoDigits(alarmHour)+":"+twoDigits(alarmMinute)).c_str(),120,68,4);
  lcarsButton(12,108,48,36,C_LAV,"H-");lcarsButton(66,108,48,36,C_ORANGE,"H+");lcarsButton(126,108,48,36,C_BLUE,"M-");lcarsButton(180,108,48,36,C_SALMON,"M+");
  lcarsButton(35,152,170,34,alarmEnabled?C_GREEN:C_RED,alarmEnabled?"ALARME LIGADO":"ALARME DESLIGADO");drawFooter();
}
void drawCamera(){
  drawHeader("CAMERA");uint16_t c=jarvisBleIsConnected()?C_GREEN:C_GOLD;tft->fillRoundRect(18,67,204,88,14,c);tft->fillRoundRect(27,76,186,70,10,C_BLACK);tft->setTextColor(C_WHITE,C_BLACK);tft->drawCentreString(cameraText.c_str(),120,93,2);tft->drawCentreString(jarvisBleIsConnected()?"REMOTO PRONTO":"CONECTE O CELULAR",120,119,1);lcarsButton(42,163,156,34,C_ORANGE,"FOTOGRAFAR");drawFooter();
}
void drawGps(){
  drawHeader("GPS");uint16_t c=jarvisBleIsConnected()?C_GREEN:C_GOLD;tft->fillRoundRect(12,65,216,106,14,c);tft->fillRoundRect(21,74,198,88,10,C_BG);tft->setTextColor(C_TEXT,C_BG);tft->drawCentreString(gpsText.c_str(),120,83,2);tft->drawCentreString(jarvisBleIsConnected()?"LOCALIZACAO VIA PHONE":"CELULAR NECESSARIO",120,108,1);lcarsButton(55,128,130,27,C_BLUE,"ATUALIZAR GPS");drawFooter();
}
...
void drawScreen(){
  if(!screenAwake||!tft)return;if(currentScreen!=SCREEN_HOME)tft->fillScreen(C_BG);
  switch(currentScreen){
    case SCREEN_HOME:drawHome();break;case SCREEN_APPS:drawApps();break;case SCREEN_VOICE:drawVoice();break;case SCREEN_ALARM:drawAlarm();break;
    case SCREEN_CAMERA:drawCamera();break;case SCREEN_GPS:drawGps();break;case SCREEN_CONTROLS:drawControls();break;case SCREEN_HEALTH:drawHealth();break;
    case SCREEN_STATUS:drawStatus();break;case SCREEN_SETTINGS:drawSettings();break;case SCREEN_CLOCK:drawClock();break;
  }
}
...
void startVoice(){
  ...
  if(jarvisBleIsConnected()){jarvisBleSendCommand("voice_capture");lastMessage="Enviando conversa para IA";}else lastMessage="Voz detectada; conecte o celular";
  drawScreen();
}
...
void requestGps(){if(jarvisBleIsConnected()){jarvisBleSendCommand("gps_request");gpsText="SOLICITANDO GPS...";}else gpsText="SEM CELULAR";drawScreen();}
...
void handleTap(int x,int y){
  ...
  if(currentScreen!=SCREEN_HOME&&y>=207){if(x<78){ScreenId s=previousScreen;previousScreen=SCREEN_HOME;currentScreen=s;drawScreen();return;}if(x<154){goHome();return;}}
  if(currentScreen==SCREEN_APPS){
    if(y>=62&&y<=111){if(x<59)navigate(SCREEN_VOICE);else if(x<117)navigate(SCREEN_ALARM);else if(x<174)navigate(SCREEN_CAMERA);else navigate(SCREEN_GPS);return;}
    if(y>=117&&y<=167){if(x<59)navigate(SCREEN_CONTROLS);else if(x<117)navigate(SCREEN_HEALTH);else if(x<174)navigate(SCREEN_STATUS);else navigate(SCREEN_SETTINGS);return;}
  } else if(currentScreen==SCREEN_VOICE){if(y>=74&&y<=170)startVoice();}
  else if(currentScreen==SCREEN_ALARM){
    if(y>=108&&y<=146){if(x<61)adjustAlarm(-1,0);else if(x<120)adjustAlarm(1,0);else if(x<177)adjustAlarm(0,-5);else adjustAlarm(0,5);}
    else if(y>=152&&y<=190){alarmEnabled=!alarmEnabled;saveSettings();drawScreen();}
  }
  else if(currentScreen==SCREEN_CAMERA){if(y>=155&&y<=202)triggerCamera();}
  else if(currentScreen==SCREEN_GPS){if(y>=120&&y<=165)requestGps();}
...
void handleGestureRelease(){
  ...
  if(dt<=TAP_MAX_MS)handleTap(touchLastX,touchLastY);lastInteraction=millis();
}

void checkAlarm(){
  if(!alarmEnabled||!watch||!watch->rtc)return;
  if(millis()-lastAlarmCheck<1000)return;lastAlarmCheck=millis();
  RTC_Date n=watch->rtc->getDateTime();
  if(n.hour==alarmHour&&n.minute==alarmMinute&&n.second<2&&lastAlarmDay!=n.day){
    lastAlarmDay=n.day;wakeDisplay();previousScreen=currentScreen;currentScreen=SCREEN_ALARM;lastMessage="ALARME";drawScreen();vibrateAlarm();
  }
}
...
void loop(){
  ...
  if(screenAwake&&millis()-lastClockRefresh>1000){lastClockRefresh=millis();if(currentScreen==SCREEN_HOME||currentScreen==SCREEN_STATUS||currentScreen==SCREEN_ALARM)drawScreen();}

  int16_t x=0,y=0;bool touching=watch->getTouch(x,y);
  if(touching){
    if(!screenAwake){wakeDisplay();drawScreen();touchActive=false;delay(220);}
    else {if(!touchActive){touchActive=true;touchStartX=x;touchStartY=y;touchLastX=x;touchLastY=y;touchStartMs=millis();}else{touchLastX=x;touchLastY=y;}lastInteraction=millis();}
