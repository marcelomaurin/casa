...
unsigned long bootMillis=0, lastClockRefresh=0, lastInteraction=0, lastStepRefresh=0, lastAlarmCheck=0;
...
// Alarmes sem delay(): o loop continua vivo enquanto vibra.
bool alarmPatternActive=false;
uint8_t alarmPulseIndex=0;
unsigned long alarmNextPulse=0;
...
void startAlarmPattern(){alarmPatternActive=true;alarmPulseIndex=0;alarmNextPulse=millis();if(alarmTone==ALARM_PHONE)jarvisBleSendCommand("alarm_sound:phone");}
void processAlarmPattern(){
  if(!alarmPatternActive||millis()<alarmNextPulse)return;
  uint8_t maxPulses=alarmTone==ALARM_SHORT?2:alarmTone==ALARM_DOUBLE?6:alarmTone==ALARM_URGENT?12:3;
  if(alarmPulseIndex>=maxPulses){alarmPatternActive=false;return;}
  if(vibrationEnabled&&watch&&watch->motor)watch->motor->onec();
  unsigned long gap=alarmTone==ALARM_URGENT?180:(alarmTone==ALARM_DOUBLE?(alarmPulseIndex%2?700:220):500);
  alarmPulseIndex++;alarmNextPulse=millis()+gap;
}
...
void wakeDisplay(){if(!watch)return;if(!screenAwake){watch->displayWakeup();watch->openBL();screenAwake=true;applyPowerMode();}lastInteraction=millis();}
void sleepDisplay(){if(!watch||!screenAwake)return;watch->closeBL();watch->displaySleep();screenAwake=false;touchActive=false;}
void toggleDisplay(){if(screenAwake)sleepDisplay();else{wakeDisplay();if(tft){} /* draw below */}}
...
void enterDeepSleep(){
  ...
  // O botão físico também acorda o relógio.
...
void beginVoiceCapture(){
  ...
  if(!initMicrophone()){voiceText="MICROFONE INDISP.";lastMessage="Falha PDM";return;}
  voiceText="OUVINDO...";lastMessage="Fale agora";voiceSum=0;voiceSamples=0;voiceCaptureUntil=millis()+1800;voiceCapturing=true;vibrateShort();
}
void processVoiceCapture(){
  ...
  if(level<60){voiceText="NAO OUVI";lastMessage="Fale mais perto";return;}
  voiceText="VOZ DETECTADA";String cmd="voice_capture:"+voiceOutputLabel();
  if(jarvisBleIsConnected()&&jarvisBleSendCommand(cmd))lastMessage="Enviado ao celular";else lastMessage="Voz captada; celular indisponivel";
}
...
void drawWifi(){
  ...
  if(wifiScanning){tft->drawCentreString("BUSCANDO REDES...",120,77,2);tft->drawCentreString("interface continua ativa",120,101,1);}
  else if(wifiNetworkCount<=0){tft->drawCentreString("Nenhuma busca",120,75,2);lcarsButton(45,112,150,36,C_BLUE,"BUSCAR");}
  else{
...
void startVideoCall(){
  ...
  lastMessage=ok?"Videochamada iniciada":"Sem conexao para chamada";drawScreen();
}
...
void startWifiScan(){wifiNetworkCount=0;wifiPage=0;wifiScanning=jarvisWifiScanStart();lastMessage=wifiScanning?"Buscando redes":"Falha ao iniciar busca";drawScreen();}
void processWifiScan(){if(!wifiScanning)return;int n=jarvisWifiScanPoll(wifiNetworks,12);if(n==-1)return;wifiScanning=false;wifiNetworkCount=max(0,n);drawScreen();}
...
void saveWifiFromKeyboard(){
  if(selectedWifi.isEmpty())return;if(wifiPassword.length()>0&&wifiPassword.length()<8){lastMessage="Senha precisa de 8+ caracteres";drawScreen();return;}
  int slot=jarvisWifiFindFreeSlot();bool ok=jarvisWifiSetProfile(slot,selectedWifi,wifiPassword);lastMessage=ok?"Wi-Fi salvo":"Falha ao salvar Wi-Fi";previousScreen=SCREEN_SETTINGS;currentScreen=SCREEN_WIFI;drawScreen();
}
...
void handleTap(int x,int y){
  ...
  else if(currentScreen==SCREEN_ALARM){if(y>=101&&y<=135){if(x<61)adjustAlarm(-1,0);else if(x<120)adjustAlarm(1,0);else if(x<177)adjustAlarm(0,-5);else adjustAlarm(0,5);}else if(y>=139&&y<=170){alarmTone=(AlarmTone)(((int)alarmTone+1)%4);saveSettings();drawScreen();}else if(y>=171&&y<=205){alarmEnabled=!alarmEnabled;saveSettings();drawScreen();}}
  ...
  else if(currentScreen==SCREEN_WIFI){if(!wifiScanning&&wifiNetworkCount<=0){if(y>=105&&y<=155)startWifiScan();}else if(!wifiScanning&&y>=60&&y<196){int row=(y-60)/34;selectWifi(wifiPage*4+row);}}
...
void handleGestureRelease(){int dx=touchLastX-touchStartX,dy=touchLastY-touchStartY;unsigned long dt=millis()-touchStartMs;int adx=abs(dx),ady=abs(dy);if(adx>=SWIPE_MIN||ady>=SWIPE_MIN){if(adx>ady){if(currentScreen==SCREEN_HOME)cycleSkin(dx>0?-1:1);}else if(currentScreen==SCREEN_WIFI&&!wifiScanning&&wifiNetworkCount>4){int pages=(wifiNetworkCount+3)/4;if(dy<0&&wifiPage<pages-1)wifiPage++;if(dy>0&&wifiPage>0)wifiPage--;drawScreen();}else if(dy<0){if(currentScreen==SCREEN_HOME)navigate(SCREEN_APPS);else if(currentScreen==SCREEN_APPS)navigate(SCREEN_SETTINGS);}else{if(currentScreen==SCREEN_APPS||currentScreen==SCREEN_SETTINGS)goHome();else if(currentScreen!=SCREEN_HOME){ScreenId s=previousScreen;previousScreen=SCREEN_HOME;currentScreen=s;drawScreen();}}lastInteraction=millis();return;}if(dt<=TAP_MAX_MS)handleTap(touchLastX,touchLastY);lastInteraction=millis();}

void checkAlarm(){if(!alarmEnabled||!watch||!watch->rtc)return;if(millis()-lastAlarmCheck<1000)return;lastAlarmCheck=millis();RTC_Date n=watch->rtc->getDateTime();if(n.hour==alarmHour&&n.minute==alarmMinute&&n.second<2&&lastAlarmDay!=n.day){lastAlarmDay=n.day;wakeDisplay();previousScreen=currentScreen;currentScreen=SCREEN_ALARM;lastMessage="ALARME";drawScreen();startAlarmPattern();}}

void processPowerButton(){
  if(!powerButtonIrq)return;powerButtonIrq=false;if(!watch||!watch->power)return;watch->power->clearIRQ();
  if(millis()-lastPowerButtonMs<300)return;lastPowerButtonMs=millis();
  if(screenAwake)sleepDisplay();else{wakeDisplay();drawScreen();}
}
...
void setup(){
  ...
  loadSettings();applyPowerMode();initMotion();jarvisBleBegin();jarvisWifiBegin();lastInteraction=millis();drawScreen();
}

void loop(){
  if(!watch){delay(250);return;}
  processPowerButton();jarvisBleLoop();jarvisWifiLoop();processWifiScan();processVoiceCapture();processAlarmPattern();checkAlarm();
  ...
  if(screenAwake&&millis()-lastClockRefresh>1000){lastClockRefresh=millis();if(currentScreen==SCREEN_HOME||currentScreen==SCREEN_STATUS||currentScreen==SCREEN_ALARM||currentScreen==SCREEN_VOICE)drawScreen();}

  // Com a tela apagada o touch não acorda o relógio; o botão físico acorda.
  if(screenAwake){int16_t x=0,y=0;bool touching=watch->getTouch(x,y);if(touching){if(!touchActive){touchActive=true;touchStartX=x;touchStartY=y;touchLastX=x;touchLastY=y;touchStartMs=millis();}else{touchLastX=x;touchLastY=y;}lastInteraction=millis();}else if(touchActive){touchActive=false;handleGestureRelease();}}

  uint16_t timeout=screenTimeoutSec;if(powerMode==POWER_ULTRA&&timeout==0)timeout=10;if(screenAwake&&timeout>0&&millis()-lastInteraction>(unsigned long)timeout*1000UL)sleepDisplay();
  if(!screenAwake&&powerMode==POWER_ULTRA&&millis()-lastInteraction>120000UL)enterDeepSleep();
  delay(powerMode==POWER_NORMAL?10:powerMode==POWER_ECO?25:50);
}
