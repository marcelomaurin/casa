...
function rules_scheduler_run_rule(PDO $pdo,array $rule,array $context,string $source):array {
    ...
    try{
        ...
        $queued=[];$errors=[];
        foreach($q->fetchAll(PDO::FETCH_ASSOC) as $a){try{$cmd=rules_enqueue_action($pdo,$a,'RULE:'.$rule['slug'],$runId,$corr);$pdo->prepare("INSERT INTO automation_rule_run_actions(run_id,rule_action_id,command_id,status) VALUES(:r,:a,:c,'QUEUED')")->execute([':r'=>$runId,':a'=>$a['id'],':c'=>$cmd]);$queued[]=$cmd;}catch(Throwable $e){$pdo->prepare("INSERT INTO automation_rule_run_actions(run_id,rule_action_id,status,erro) VALUES(:r,:a,'FAILED',:e)")->execute([':r'=>$runId,':a'=>$a['id'],':e'=>substr($e->getMessage(),0,1000)]);$errors[]=['action_id'=>(int)$a['id'],'error'=>$e->getMessage()];}}
