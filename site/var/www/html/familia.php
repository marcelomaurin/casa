...
if(isset($_GET['api'])){
  ...
  if($api==='call_start'){
    $mode=($in['mode']??'video')==='audio'?'audio':'video';$s=$pdo->prepare("INSERT INTO family_calls(canal_id,iniciado_por,modo,status) VALUES(:c,:u,:m,'chamando')");$s->execute([':c'=>$channel['id'],':u'=>$user,':m'=>$mode]);$id=(int)$pdo->lastInsertId();family_send($pdo,(int)$channel['id'],$user,'web','call','Chamada familiar iniciada',['call_id'=>$id,'mode'=>$mode]);echo json_encode(['ok'=>true,'call_id'=>$id]);exit;
  }
  if($api==='call_current'){
    $s=$pdo->prepare("SELECT * FROM family_calls WHERE canal_id=:c AND status IN('chamando','ativa') ORDER BY id DESC LIMIT 1");$s->execute([':c'=>$channel['id']]);echo json_encode(['ok'=>true,'call'=>$s->fetch(PDO::FETCH_ASSOC)?:null],JSON_UNESCAPED_UNICODE);exit;
  }
  if($api==='call_join'){
    $id=(int)($in['call_id']??0);$pdo->prepare("UPDATE family_calls SET status='ativa' WHERE id=:id AND canal_id=:c AND status='chamando'")->execute([':id'=>$id,':c'=>$channel['id']]);echo json_encode(['ok'=>true]);exit;
  }
  if($api==='signal'){
...
async function joinCurrent(){const r=await api('call_current');if(!r.call)return;callId=+r.call.id;document.getElementById('callStatus').textContent='Chamada '+r.call.modo+' #'+callId;await api('call_join',{call_id:callId});await getMedia(r.call.modo);await sendSignal('join',{mode:r.call.modo})}
...
setInterval(()=>{presence();poll();pollSignals()},2000);presence();poll();joinCurrent().catch(()=>{});
</script></body></html>