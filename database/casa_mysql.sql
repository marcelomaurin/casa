...
-- Banco Hostinger: u820932905_casa
-- Usuario MySQL: u820932905_mmaurin
-- Gerado para substituir os esquemas PostgreSQL legados do projeto.
--
-- IMPORTANTE:
-- 1. O banco e o usuario ja devem existir no hPanel da Hostinger.
-- 2. Este arquivo NAO cria usuario MySQL, NAO concede privilegios e NAO grava senha.
-- 3. Se o hPanel nao permitir USE, selecione u820932905_casa no phpMyAdmin e remova
--    apenas a linha USE abaixo.
-- 4. Tokens, senhas Wi-Fi e chaves privadas nao devem ser inseridos neste arquivo.
-- ============================================================================
...
('ia_cloud_keywords', 'analise,pesquisa,programe,codigo,explique,calcule,redija,artigo,relatorio,python,sql,resolva,resuma,estude,arquitetura,desenvolva', 'Palavras-chave para roteamento de IA'),
...
-- O primeiro administrador e criado pelo login.php usando a variavel privada
-- JARVIS_BOOTSTRAP_ADMIN_PASSWORD. Nenhuma senha padrao e gravada neste SQL.

-- --------------------------------------------------------------------------
-- DISPOSITIVOS LEGADOS / PARAMETROS
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- ARQUITETURA DISTRIBUIDA: NOS ARM / RASPBERRY
-- --------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS arm_nodes (
    ...
    KEY idx_arm_nodes_ip (ip_address),
    KEY idx_arm_nodes_status (status, ultimo_ping)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- --------------------------------------------------------------------------
-- CLUSTER IOT / DEVICES DISTRIBUIDOS
-- --------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS camera_eventos (
    ...
    KEY idx_camera_faces (quantidade_faces, data_hora),
    KEY idx_camera_status (status_processamento, data_hora),
    KEY idx_camera_status_ia (status_ia, data_hora),
    CONSTRAINT fk_camera_dispositivo FOREIGN KEY (id_dispositivo) REFERENCES dispositivos_cluster(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- --------------------------------------------------------------------------
-- VOZ / FRASES / CONVERSAS / LOGS
-- --------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS seguranca_logs (
    ...
    KEY idx_seg_logs_evento_data (evento, data_hora)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- --------------------------------------------------------------------------
-- API V1 / CLIENTES / RATE LIMIT / AUDITORIA
-- --------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS api_v1_rate_limit (
    ...
    atualizado_em TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (chave),
    KEY idx_rate_bloqueado (bloqueado_ate)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS api_v1_security_log (
    ...
    KEY idx_api_security_evento (evento, data_hora)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS internet_pesquisas (
    ...
    KEY idx_internet_status_data (status, data_hora)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS tarefas_agendadas (
    ...
    KEY idx_tarefas_executar_em (executar_em)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- --------------------------------------------------------------------------
-- PLANOS / TAREFAS DA IA
-- --------------------------------------------------------------------------
...
ALTER TABLE tarefas_agendadas
    ADD CONSTRAINT fk_tarefa_plano FOREIGN KEY (id_plano) REFERENCES jarvis_planos(id) ON DELETE SET NULL,
    ADD CONSTRAINT fk_tarefa_item FOREIGN KEY (id_tarefa_plano) REFERENCES jarvis_tarefas(id) ON DELETE SET NULL;
...
-- Nenhum device token real e inserido. Devices devem ser cadastrados pela API/painel.
-- Nenhum system_api_token e inserido. Configure JARVIS_SYSTEM_API_TOKEN fora do Git.
-- Nenhuma senha de usuario e inserida.

SET FOREIGN_KEY_CHECKS = 1;

SELECT 'CASA/JARVIS MySQL schema instalado' AS status,
       DATABASE() AS banco,
       CURRENT_TIMESTAMP AS instalado_em;
